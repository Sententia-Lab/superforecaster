...
Each tool takes `RunContext[ForecastDeps]` and reads `ctx.deps.as_of`. When it is set, the
...
When `as_of` is None the tools return current results with no filtering. That is the
production path.
