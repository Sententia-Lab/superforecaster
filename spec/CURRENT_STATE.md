...
│   │   ├── __init__.py               # Loads .env at import
...
See `.env.example` at repo root. Backend reads from `backend/.env` first, then root `.env`.
...
- `.env.example` at repo root — covers all backend + frontend variables
