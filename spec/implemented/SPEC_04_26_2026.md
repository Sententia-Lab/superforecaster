...
> **Implemented and merged (v3).** Kept as a record of how the platform was built. The
> `/ralph-loop` commands below reference `/Users/narenchaudhry/personal/superforecaster/SPEC.md`,
> a path that no longer exists — this file *is* that spec, since moved. The agent architecture
> it describes was later replaced; see `spec/implemented/spec3.md` and `spec/ADR.md` §10–11.
> For what the code looks like today, read `spec/CURRENT_STATE.md`.
